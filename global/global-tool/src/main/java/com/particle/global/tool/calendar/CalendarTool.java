...
public class CalendarTool {
    ...
    public static final String CALENDAR_DATEFORMAT_THREAD_LOCAL_KEY = "CALENDAR_DATEFORMAT_THREAD_LOCAL_KEY";
    public static final String CALENDAR_DATEFORMAT_THREAD_LOCAL_LENIENT_KEY = "CALENDAR_DATEFORMAT_THREAD_LOCAL_LENIENT_KEY";
    ...
    private static SimpleDateFormat getDateFormat(String pattern) throws RuntimeException {
        SimpleDateFormat dateFormat = (SimpleDateFormat) ThreadContextTool.get(CALENDAR_DATEFORMAT_THREAD_LOCAL_KEY);
        if (dateFormat == null) {
            synchronized (OBJECT) {
                if (dateFormat == null) {
                    dateFormat = new SimpleDateFormat(pattern);
                    dateFormat.setLenient(false);
                    ThreadContextTool.put(CALENDAR_DATEFORMAT_THREAD_LOCAL_KEY,dateFormat);
                }
            }
        }
    ...
    private static SimpleDateFormat getDateFormat(String pattern,boolean lenient) throws RuntimeException {
        if(lenient){
            SimpleDateFormat dateFormat = (SimpleDateFormat) ThreadContextTool.get(CALENDAR_DATEFORMAT_THREAD_LOCAL_LENIENT_KEY);
            if (dateFormat == null) {
                synchronized (OBJECT) {
                    if (dateFormat == null) {
                        dateFormat = new SimpleDateFormat(pattern);
                        dateFormat.setLenient(lenient);
                        ThreadContextTool.put(CALENDAR_DATEFORMAT_THREAD_LOCAL_LENIENT_KEY,dateFormat);
                    }
                }
            }
            dateFormat.applyPattern(pattern);
    ...
    public static String dateToString(LocalDateTime date) {
        DateTimeFormatter f = DateTimeFormatter.ofPattern(DateStyle.YYYY_MM_DD.value);
    ...
    public static int getIntervalDays(Date date, Date otherDate) {
        int num = 0;
        Date dateTmp = CalendarTool.stringToDate(CalendarTool.getDate(date), DateStyle.YYYY_MM_DD);
        Date otherDateTmp = CalendarTool.stringToDate(CalendarTool.getDate(otherDate), DateStyle.YYYY_MM_DD);
