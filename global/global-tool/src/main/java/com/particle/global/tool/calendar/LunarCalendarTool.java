...
public class LunarCalendarTool {
     ...
     */
    private final static int[] SOLAR_TERM_24_INFO = {
     ...
     * 支持的阴历结束年份
     */
    final static int END_YEAR = 2049;
    /**
     * 农历开始日期
     * 日柱：甲辰
     */
    final static String LUNAR_DATE_START = START_YEAR + "-01-01";
    /**
     * 公历开始日期
     * 农历1900-01-01和公历1900-01-31是同一天
     */
    final static String CAL_DATE_START = START_YEAR + "-01-31";
    /**
     * 节气参考值，该日期是小寒
     */
    final static String SOLAR_TERM_REFER = START_YEAR + "-01-06 02:05:00";
    ...
    final public static int leapMonth(int lunarYear) {
        return (int) (LUNAR_INFO[lunarYear - START_YEAR] & 0xf);
    }
    ...
    final public static int leapMonthDays(int lunarYear) {
        if (leapMonth(lunarYear) != 0) {
            if ((LUNAR_INFO[lunarYear - START_YEAR] & 0x10000) != 0){
                return 30;
            }
            else{
                return 29;
    ...
    final public static int yearDays(int lunarYear) {
        ...
        for (i = 0x8000; i > 0x8; i >>= 1) {
            if ((LUNAR_INFO[lunarYear - START_YEAR] & i) != 0){
                sum += 1;
            }
        }
    ...
    final public static int monthDays(int lunarYear, int lunarMonth) {
        if ((LUNAR_INFO[lunarYear - START_YEAR] & (0x10000 >> lunarMonth)) == 0) {
            return 29;
        }
        else{
            return 30;
    ...
    final public static Tiangan tianganYear(int lunarYear){
        ...
        int num = lunarYear - START_YEAR + 36;
    ...
    final public static Tiangan tianganMonth(int lunarYear,int lunarmonth){
        Tiangan tiangan = LunarCalendarTool.tianganYear(lunarYear);
    ...
    final public static Tiangan tianganDay(int lunarYear,int lunarMonth,int lunarDay){
        ...
        int num = CalendarTool.getIntervalDays(LunarToCalendar(date), CalendarTool.stringToDate(CAL_DATE_START));
        if(num >= 0){
            num = num  % 10 + 1;
        }else{
            num = 10 - (num  % 10 + 1);
    ...
    final public static Tiangan tianganHour(int lunarYear,int lunarMonth,int lunarDay,int lunarHour,int lunarMinite){
        ...
        // 最后得到天干
        int rNum = (firstHourNum + numLunar) % 10;
        if(rNum == 0){
            rNum = 10;
        }
    ...
    public static LunarDate CalendarToLunar(Date date) {
        ...
        int leapMonth = 0;
        Date baseDate = CalendarTool.stringToDate(CAL_DATE_START);
        Date nowaday=date;
        ...
        for (iYear = START_YEAR; iYear < 10000 && offset > 0; iYear++) {
        ...
        int second = CalendarTool.getSecond(date);
    ...
    public static Date LunarToCalendar(LunarDate lunarDate){
        ...
        for(int y = START_YEAR; y < year; y++){ //计算整年天数
            dt += yearDays(y);
        }
        ...
        for(int i = 1,d = 0x8000;i < mon; i++, d>>=1) {
            dt += (LUNAR_INFO[year- START_YEAR] & d) == 0 ? 29: 30;
        }
        //当月天数
        dt += day;
        ...
        Date date = CalendarTool.stringToDate(CAL_DATE_START + " "+hour + ":" + minute + ":" + second);
        return CalendarTool.addDay(date,dt - 1);
    }
    ...
    private static Date getSolarTerm24Date(int year, int number) {
        long l = (long) 31556925974.7 * (year - START_YEAR)
                + SOLAR_TERM_24_INFO[number-1] * 60000L;
        Date tempDate = CalendarTool.stringToDate(SOLAR_TERM_REFER, CalendarTool.DateStyle.YYYY_MM_DD_HH_MM_SS);
    ...
    public static SolarTerm24 getSolarTerm24(Date date){
        ...
        int year = Integer.parseInt(CalendarTool.dateToString(date, "yyyy"));
        ...
        for (SolarTerm24 solarTerm24 : SolarTerm24.values()) {
            ...
            map.put(CalendarTool.dateToString(tempDate, CalendarTool.DateStyle.YYYY_MM_DD), solarTerm24);
        }

        return map.get(CalendarTool.dateToString(date, CalendarTool.DateStyle.YYYY_MM_DD));
    }
    ...
    public static WeekSolarHoliday getWeekHoliday(Date date){
        CalendarTool.Week week = CalendarTool.getWeek(date);
        int month = CalendarTool.getMonth(date);
        for (WeekSolarHoliday weekSolarHoliday : WeekSolarHoliday.values()) {
            // 如果月份和星期几都相同
            if(month == weekSolarHoliday.getMonth() && week.getNumber() == weekSolarHoliday.getDay() &&  CalendarTool.getWeekOfMonth(date) == weekSolarHoliday.getWhichWeek()){
    ...
    public static SolarHoliday getSolarHoliday(Date date){
        for (SolarHoliday solarHoliday : SolarHoliday.values()) {
            if(solarHoliday.getMonthday().equals(CalendarTool.dateToString(date, CalendarTool.DateStyle.MM_DD))){
                return solarHoliday;
            }
        }
        return null;
     ...
     * @return
     */
    public static String lunarDateToString(LunarDate date, CalendarTool.DateStyle dateStyle){
        if(date.getMonth() == 2 && date.getDay() <=  28){
            ...
            return CalendarTool.dateToString(CalendarTool.stringToDate(lunarDateStr,true),dateStyle,true);
        }else{
    ...
    public static LunarDate stringTolunarDate(String date){
        LunarDate lunarDate = strTolunarDate(date);
        if(lunarDate.getMonth() == 3 && lunarDate.getDay() == 1){
            ...
            LunarDate lunarDate1 = strTolunarDate(lunarDateStr1);
            ...
            LunarDate lunarDate2 = strTolunarDate(lunarDateStr2);
     ...
     * @return
     */
    private static LunarDate strTolunarDate(String date){


        Date stringToDate = CalendarTool.stringToDate(date,true);
        int year = CalendarTool.getYear(stringToDate);
        int month = CalendarTool.getMonth(stringToDate);
        int day = CalendarTool.getDay(stringToDate);
        int hour = CalendarTool.getHour(stringToDate);
        int minute = CalendarTool.getMinute(stringToDate);
        int second = CalendarTool.getSecond(stringToDate);
        return new LunarDate(year,month,day,hour,minute,second);
    }
}
