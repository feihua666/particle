 ...
 * 但尽量不要在 service 中过度使用或者就干脆不使用为好，尽量保持方法调用登录用户参数传递
...
public class LoginUserTool {
    ...
    public static List<Long> getLoginUserRoleIds(){
        ...
        for (UserGrantedAuthority authority : userGrantedAuthorities) {
            Optional.ofNullable((authority).getGrantedPermissionRole()).ifPresent(grantedRole -> {
    ...
    public static List<Long> getLoginUserPermissionIds(){
        ...
        Set<Long> result = new HashSet<>();
        for (UserGrantedAuthority authority : userGrantedAuthorities) {
            Optional.ofNullable((authority).getGrantedPermission()).ifPresent(grantedPermission -> {
                if (grantedPermission.getId() != null) {
