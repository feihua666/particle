...
CREATE TABLE `component_data_query_data_api` (
  ...
  `is_master` tinyint(1) NOT NULL COMMENT '是否为主版本，非主版本视为开发版本',
  `master_id` bigint NULL COMMENT '主版本id',
  `is_test_passed` tinyint(1) NOT NULL COMMENT '是否测试通过，测试通过才能发布',
  `data_query_provider_id` bigint DEFAULT NULL COMMENT '数据查询供应商id,用于支持开放接口',
  `api_identifier` varchar(100) DEFAULT NULL COMMENT '接口标识，一般与url对等,用于支持开放接口',
  `api_version` varchar(100) DEFAULT NULL COMMENT '接口版本标识，标识同一个接口的不同版本,用于支持开放接口',
  `is_support_warehouse` tinyint(1) NOT NULL COMMENT '是否支持入库,用于支持开放接口',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin COMMENT='数据查询数据接口表';
