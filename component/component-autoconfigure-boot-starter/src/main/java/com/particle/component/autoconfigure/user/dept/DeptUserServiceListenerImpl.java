 ...
 */
public class DeptUserServiceListenerImpl implements IAddServiceListener<UserDO>, IUpdateServiceListener<UserDO>, IQueryWrapperHandler<UserDO> {
		...
		if (po.getUpdateControl() instanceof UserUpdateCommand) {
			...
			if (userUpdateCommand.getDeptId() == null) {
				...
				if (setNullWhenNull != null) {
    				//	删除用户的部门
                    CommonIdCommand deleteCommand = CommonIdCommand.create(po.getId());
