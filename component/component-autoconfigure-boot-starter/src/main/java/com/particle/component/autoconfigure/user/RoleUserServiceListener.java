 ...
 */
public class RoleUserServiceListener implements IAddServiceListener<UserDO> , IUpdateServiceListener<UserDO>, IQueryWrapperHandler<UserDO> {
	...
	public void handle(QueryWrapper<UserDO> queryWrapper, QueryCommand queryForm) {
        ...
        if (StrUtil.isNotEmpty(roleName)) {
            ...
            if (CollectionUtil.isEmpty(roleVOS)) {
                ...
                // 存在根据角色id查询，拼接查询条件
                BatchIdCommand batchIdCommand = BatchIdCommand.create(roleIds);
                MultiResponse<RoleUserRelVO> roleUserRelVOMultiResponse = roleUserRelRpcFeignClient.queryListByRoleIds(batchIdCommand);
                List<RoleUserRelVO> roleUserRelVOS = roleUserRelVOMultiResponse.getData();
                List<Long> userIds = roleUserRelVOS.stream().map(RoleUserRelVO::getUserId).collect(Collectors.toList());
                if (userIds.isEmpty()) {
                    // 为空就是不存在，不存在，得加一个不存在的条件
