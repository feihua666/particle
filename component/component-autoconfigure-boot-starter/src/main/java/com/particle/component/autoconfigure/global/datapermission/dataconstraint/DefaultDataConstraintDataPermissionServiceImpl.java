...
import com.particle.global.dto.login.DeptInfo;
import com.particle.global.dto.login.GrantedDataConstraint;
 ...
 * <p>
 * 基于数据范围约束的数据权限实现
 * 主要是根据数据范围约束条件，动态处理 queryWrapper 查询条件
 * 该数据权限实现主要根据 部门 归属数据思路实现
 * </p>
 *
 * @author yangwei
...
public class DefaultDataConstraintDataPermissionServiceImpl implements DataPermissionService {
    ...
    private static SecurityUserDeptService securityUserDeptServiceStatic;
    ...
    public AbstractWrapper dataConstraint(AbstractWrapper wrapper, DataConstraintContext dataConstraintContext) {
        LoginUser loginUser = com.particle.global.tool.login.LoginUserTool.getLoginUser();
        // 用户为空不限制
        if (loginUser == null) {
            return wrapper;
        }
    ...
    static {
        ...
        builtInConstraintContentPrefixWithSuffixColumn.put("selfAssignedDeptPrefix", (wrapper,loginUser,suffixColumnName) -> {
            Assert.notNull(securityUserDeptServiceStatic,"不支持的数据范围约束 selfAssignedDeptPrefix，请检查配置信息");

            List<DeptInfo> deptInfos = securityUserDeptServiceStatic.retrieveDeptInfoByDataConstraint();
    ...
    public void setUserDeptService(SecurityUserDeptService securityUserDeptService) {
        this.securityUserDeptService = securityUserDeptService;
        this.securityUserDeptServiceStatic = securityUserDeptService;
    }
}
