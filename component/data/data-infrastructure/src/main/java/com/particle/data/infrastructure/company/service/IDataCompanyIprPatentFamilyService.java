...
public interface IDataCompanyIprPatentFamilyService extends IBaseService<DataCompanyIprPatentFamilyDO> {
     ...
     * @return
     */
    default public DataCompanyIprPatentFamilyDO getByCompanyIprPatentIdAndStandardApplyNo(Long companyIprPatentId,String standardApplyNo) {
        Assert.notNull(companyIprPatentId,"companyIprPatentId 不能为空");
        Assert.notEmpty(standardApplyNo,"standardApplyNo 不能为空");
        return getOne(Wrappers.<DataCompanyIprPatentFamilyDO>lambdaQuery()
                .eq(DataCompanyIprPatentFamilyDO::getCompanyIprPatentId, companyIprPatentId)
                .eq(DataCompanyIprPatentFamilyDO::getStandardApplyNo, standardApplyNo)
        );
    }
    /**
     * 根据 专利id 和 原始公布号 查询
     ...
     * @return
     */
    default public DataCompanyIprPatentFamilyDO getByCompanyIprPatentIdAndStandardPublicNo(Long companyIprPatentId,String standardPublicNo) {
        Assert.notNull(companyIprPatentId,"companyIprPatentId 不能为空");
        Assert.notEmpty(standardPublicNo,"standardPublicNo 不能为空");
        return getOne(Wrappers.<DataCompanyIprPatentFamilyDO>lambdaQuery()
                .eq(DataCompanyIprPatentFamilyDO::getCompanyIprPatentId, companyIprPatentId)
                .eq(DataCompanyIprPatentFamilyDO::getStandardPublicNo, standardPublicNo)
        );
    }
    /**
     * 更新最后处理时间
