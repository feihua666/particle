package com.particle.data.client.company.api;

import com.particle.common.client.api.IBaseApplicationService;
import com.particle.common.client.dto.command.CommonIdCommand;
import com.particle.global.dto.response.SingleResponse;
import com.particle.data.client.company.dto.command.DataCompanyVcProductCreateCommand;
import com.particle.data.client.company.dto.command.DataCompanyVcProductUpdateCommand;
import com.particle.data.client.company.dto.data.DataCompanyVcProductVO;
import com.particle.data.client.company.dto.command.warehouse.DataCompanyVcProductWarehouseCommand;
import com.particle.data.client.company.dto.data.exwarehouse.DataCompanyVcProductExWarehouseVO;
/**
 * <p>
 * 企业融资产品 应用门面服务类
 * </p>
 *
 * @author yw
 * @since 2025-04-05 16:47:14
 */
public interface IDataCompanyVcProductApplicationService extends IBaseApplicationService {
	/**
	 * 添加/创建一个领域对象
	 * @param dataCompanyVcProductCreateCommand
	 * @return
	 */
	SingleResponse<DataCompanyVcProductVO> create(DataCompanyVcProductCreateCommand dataCompanyVcProductCreateCommand);

	/**
	 * 删除领域对象
	 * @param deleteCommand
	 * @return
	 */
	SingleResponse<DataCompanyVcProductVO> delete(CommonIdCommand deleteCommand);

	/**
	 * 更新领域对象
	 * @param dataCompanyVcProductUpdateCommand
	 * @return
	 */
	SingleResponse<DataCompanyVcProductVO> update(DataCompanyVcProductUpdateCommand dataCompanyVcProductUpdateCommand);

	/**
	 * 企业融资产品入库
	 * @param dataCompanyBasicWarehouseCommand
	 * @return
	 */
	SingleResponse<DataCompanyVcProductExWarehouseVO> warehouse(DataCompanyVcProductWarehouseCommand dataCompanyBasicWarehouseCommand);
}
