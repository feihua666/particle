DROP TABLE IF EXISTS component_lowcode_segment_template;
CREATE TABLE `component_lowcode_segment_template` (
  ...
  `content_template` varchar(5000) DEFAULT NULL COMMENT '内容模板',
  `reference_segment_template_id` bigint DEFAULT NULL COMMENT '引用模板id',
  `output_type` varchar(255) DEFAULT NULL COMMENT '输出类型，file=文件，dir=目录，segment=片段',
  `output_variable` varchar(255) DEFAULT NULL COMMENT '输出变量名',
  `share_variables` varchar(255) DEFAULT NULL COMMENT '共享变量名，多个以逗号分隔，变量类型为List<String>',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='低代码片段模板表';
