 ...
 */
@FeignClient(name = "${particle.feign-client.dataconstraint.name:dataconstraint-start}", contextId = "dataScopeTransRpcFeignClient", url = "${particle.feign-client.dataconstraint.url:}", path = "/rpc/data_scope")
public interface DataScopeTransRpcFeignClient extends ITransService<DataScopeTransVO,Long> {
    ...
    @Override
    default public TransResult<DataScopeTransVO, Long> trans(@RequestParam String type, @RequestParam Long key) {
        return null;
    }
}
