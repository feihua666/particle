package com.particle.workflow.adapter.execution.mobile.admin;

import com.particle.common.adapter.mobile.AbstractBaseMobileAdapter;
import  com.particle.workflow.client.execution.api.IWorkflowExecutionApplicationService;
import io.swagger.v3.oas.annotations.tags.Tag;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

/**
 * <p>
 * 工作流执行实例后台管理移动端前端适配器
 * 主要用于移动端后台管理
 * </p>
 *
 * @author yw
 * @since 2026-04-28 09:57:51
 */
@Tag(name = "工作流执行实例移动端后台管理相关接口")
@RestController
@RequestMapping("/admin/mobile/workflow_execution")
public class WorkflowExecutionAdminMobileController extends AbstractBaseMobileAdapter {

	@Autowired
	private IWorkflowExecutionApplicationService iWorkflowExecutionApplicationService;


}