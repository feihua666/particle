...
let prefix = getApiPrefix(import.meta.env.VITE_API_PREFIX_DATA_QUERY) + '/admin/web/data_query_data_api'
/**
 * 添加数据查询数据接口
 * @param data
 */
...
export const copydev = (data: anyObj): AxiosPromise => {
    return axios.post(prefix + '/copydev',data)
}
/**
 * dev提交到master
 * @param data
...
export const devMergeToMaster = (data: anyObj): AxiosPromise => {
    return axios.put(prefix + '/devMergeToMaster',data)
}
/**
 * 删除数据查询数据接口
 * @param data
...
export const deleteCache = (data: IdParam): AxiosPromise => {
    return axios.delete(prefix + '/deleteCache',{data: data})
}
/**
 * 刷新执行器供应商缓存
 * @param data
...
export const refreshOpenapiExecuteProviderCache = (): AxiosPromise => {
    return axios.put(prefix + '/refreshOpenapiExecuteProviderCache')
}
/**
 * 刷新缓存数据查询数据接口
 * @param data
 ...
 * @param data
 */
const dataQueryDataApiTestPrefix = prefix + '/test'
